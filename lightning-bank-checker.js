window.BankChecker = function(component) {
    return {
        checkAccount : function() {
            //check with payments nz
            var checkWithPaymentsNZ = true;
            var bankId = component.find('bankid').get('v.value');
            var branchId = component.find('branchid').get('v.value');
            var accountBaseId = component.find('accountbaseid').get('v.value');
            var suffixId = component.find('suffixid').get('v.value');   
            //data retrieved from cache
            var jsonObj = JSON.parse(window.DataCache.getData('bbr'));
            //if enabled check with payments nz for active/new branches
            if(checkWithPaymentsNZ && bankId.length > 1 && branchId.length > 3) {
            ...
            if(accountBaseId.length > 0 && suffixId.length >1) {
